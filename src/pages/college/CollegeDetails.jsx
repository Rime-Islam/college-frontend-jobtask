  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center">
        <img
          src={college?.image?.location || "https://images.unsplash.com/photo-1541339907198-e08756dedf3f?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80"}
          alt={college?.name}
        ...
        <div className="flex-1 space-y-2">
          ...
          <p className="text-yellow-500 font-semibold">Rating: {college?.rating}/5</p>
        </div>
      </div>
      ...
      <div className="border p-4 rounded-lg bg-gray-50">
        ...
        <p>
          <strong>Start:</strong> {new Date(college?.admissionDates?.startDate).toLocaleDateString()}
        </p>
        <p>
          <strong>End:</strong> {new Date(college?.admissionDates?.endDate).toLocaleDateString()}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="border p-4 rounded-lg bg-gray-50 space-y-1">
          ...
          <p>
            <MapPin className="inline mr-1" /> {college?.location?.city}, {college?.location?.country}
          </p>
        </div>
        <div className="border p-4 rounded-lg bg-gray-50 space-y-1">
          ...
          {college?.contact?.website && (
            <p>
              ...
              <a href={college?.contact?.website} target="_blank" className="text-blue-600">
                {college?.contact?.website}
              </a>
            </p>
          )}
          ...
          <div className="space-y-4">
            {college?.researchHistory?.map((research, index) => (
              <div key={index} className="border p-4 rounded-lg bg-gray-50">
                ...
                <p>
                  <strong>Paper:</strong>{" "}
                  <a href={research?.paperLink} target="_blank" className="text-blue-600">
                    {research?.paperLink}
                  </a>
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {college?.sports?.map((sport, index) => (
              <div key={index} className="border p-4 rounded-lg bg-gray-50">
                ...
                {sport?.coachName && <p><strong>Coach:</strong> {sport?.coachName}</p>}
                {sport?.achievements.length > 0 && (
                  <p>
                    <strong>Achievements:</strong> {sport?.achievements?.join(", ")}
                  </p>
                )}
              </div>
            ))}
